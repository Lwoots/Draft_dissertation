...
Reads were assembled using the pipeline HybPiper 1.3.1 (ref).  HybPiper consists of a series of python scripts that use SPAdes (ref) to assemble reads to a target file. The pipeline was run using BLAST to recover the coding genes that matched a multi-species target 
