...
The alignments were concatenated using AMAS (ref), which also generates a file delimiting where each gene starts in the concatenated alignment. Each gene is expect to have a different rate and pattern of evolution. Therefore each gene should be fitted with a best fit model. IQtree is able to fit 200 odd models of evolution to the genes. To reduce parameter space, it is best to join genes with similar models of information. IQtree can implement Lanfear's partitionfinder2. Due to the high computational burden, I choose to use a less strict searching algorithm rcluster, which ... 

I then ran IQtree with 1000 ultrafast bootstraps, and -alrt 1000. With these support metrics, a node is considered to have good support if ...
...
As coalescent-based methods are less sensitive to missing data, I used only the ninety-percent coverage dataset for this analysis 
