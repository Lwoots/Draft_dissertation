...
```{r rehmannii_phylogeny, echo=F, warning=F, fig.width=10, fig.height=9, fig.cap="Phylogenetic relationships of Rehmannii clade. A) ML tree, support values UFBootstraps, B) ASTRAL tree, support values quartet scores"}
...
```{r rehmannii_gbs, echo=F, warning=FALSE, cache=T, message=F, include = F, fig.height=3.2}
...
```{r, fig.height=6.4}
#ggpubr::ggarrange(grid.arrange(p3$plot[[1]]), grid.arrange(p4$plot[[1]]), nrow = 2)

#cowplot::plot_grid(grid.arrange(p3$plot[[1]]), grid.arrange(p4$plot[[1]]), nrow = 2)
```
...
```{r pca_reh, echo=F, warning=FALSE, cache=T, message=F, fig.height=7}
...
The Ramosa clade consists of three subgroups, here called Aphylla, Eastern ramosa and Western ramosa, in contrast to the  
